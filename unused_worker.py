...
def run_worker(job_log, topo, delegate_queue, submitted_job_queue, fin_job_num):
    ...
    is_mid_time_to_flush_sdc_advanced = (
        (t_flush > (flush_period / 2))
        and (is_mid_checked == False)
        and scheduling_policy == "sdc_advanced"
    )
    is_time_to_flush_sdc_advanced = (t_flush > flush_period) and (
        scheduling_policy == "sdc_advanced"
    )
    is_time_to_flush_others = (t_flush > flush_period) and (
        scheduling_policy != "sdc_advanced"
    )
    ...
    if is_mid_time_to_flush_sdc_advanced:
        ...
        execute_scheduling_policy(
            topo, delegate_queue, submitted_job_queue, scheduling_policy, fin_job_num
        )
...
def execute_scheduling_policy(
    ...
    if scheduling_policy == "fcfs":
        ...
        if len(delegate_queue) > 1:
            req_num_cores_list = list(
                map(lambda x: x.required_num_cores, delegate_queue)
            )
            ...
            if min_core < max_core:
                normalized_req_num_cores_list = list(
                    map(
                        lambda x: (x - min_core) / (max_core - min_core),
                        req_num_cores_list,
                    )
                )
            else:
                normalized_req_num_cores_list = list(
                    map(lambda x: 0.5, req_num_cores_list)
                )

            pending_time_list = list(
                map(
                    lambda x: (cur_time_ms() - x.submitted_time) / flush_period,
                    delegate_queue,
                )
            )
            ...
            if min_pending_time < max_pending_time:
                normalized_pending_time_list = list(
                    map(
                        lambda x: (x - min_pending_time)
                        / (max_pending_time - min_pending_time),
                        pending_time_list,
                    )
                )
            else:
                normalized_pending_time_list = list(
                    map(lambda x: 0.5, pending_time_list)
                )
            ...
            for i in range(len(delegate_queue)):
                score = (
                    normalized_req_num_cores_list[i] * normalized_pending_time_list[i]
                )
            ...
            for i in range(len(delegate_queue)):
                delegate_queue[i].score = (
                    lambda_val * normalized_req_num_cores_list[i]
        ...
        if len(ou_queue) == 2:
            flush_period = (
                flush_period * 0.5
                if (not ou_queue[0] or not ou_queue[1])
                else ou_queue[1] / ou_queue[0]
            )
...
def init_virt_topo(topo):
    ...
    for i in range(len(topo)):
        ...
        for j in range(len(topo[i])):
            host = topo[i][j]
            virt_host = Host(
                host.host_type, host.queue_type, host.num_cores, host.avail_num_cores
            )
...
def two_stage_process(virt_topo, delegate_queue, submitted_job_queue, fin_job_num):
    ...
    for i in range(len(delegate_queue)):
        ...
        for j in range(len(virt_topo[target_queue_type])):
            if (
                virt_topo[target_queue_type][j].avail_num_cores
                >= delegate_queue[i].required_num_cores
            ) and (virt_topo[target_queue_type][j].avail_num_cores <= min_val):
                min_val = virt_topo[target_queue_type][j].avail_num_cores
                min_idx = j

        if min_idx != -1:
            virt_topo[target_queue_type][min_idx].avail_num_cores -= delegate_queue[
                i
            ].required_num_cores
    ...
    for i in range(len(temp_vec_b)):
        ...
        for j in range(len(virt_topo)):
            ...
            for k in range(len(virt_topo[j])):
                if (
                    virt_topo[j][k].avail_num_cores >= temp_vec_b[i].required_num_cores
                ) and (virt_topo[j][k].avail_num_cores <= min_val):
                    min_val = virt_topo[j][k].avail_num_cores
                    min_idx_queue = j
                    min_idx_host = k

        if min_idx_queue != -1:
            virt_topo[min_idx_queue][min_idx_host].avail_num_cores -= temp_vec_b[
                i
            ].required_num_cores
